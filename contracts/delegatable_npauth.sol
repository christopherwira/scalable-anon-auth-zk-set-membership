...
import '../temp/contracts/delegatable_npauth_verifier.sol';

contract DelegatableNPAuth is Verifier {
    uint constant INPUT_LENGTH = 14;
    uint constant ADDRESS_ACCOUNT_LENGTH = 5;
    bytes32 public current_merkle_root;
    mapping(address => bool) is_submitted;
    ...
    function check_replay_attack(address statement_agent) public returns(bool) {
        if(statement_agent != msg.sender){
            return false;
        }
        if(is_submitted[statement_agent]){
            return false;
        }
        is_submitted[statement_agent] = true;
        return true;
    }
